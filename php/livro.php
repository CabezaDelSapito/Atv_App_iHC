<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
        ...
        if(isset($_GET['id'])){
            ...
            $id = $_GET['id'];
            $sql = $con->query("select * from livro where id='$id'");
            $data = $sql->fetch_assoc();
        }else{
            // Entra nesse caso não tenha passagem de ID via "get"
            $data = array();
            $sql = $con->query("select * from livro");
    ...
    if($_SERVER['REQUEST_METHOD'] === 'PUT'){
        ...
        if(isset($_GET['id'])){
            ...
            $sql = $con->query("update livro set
                titulo = '".$data->titulo."',
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        // gravar informações
        $data = json_decode(file_get_contents("php://input"));
        $sql = $con->query("insert into livro(titulo, categoria, autor, descricao) values ('".$data->titulo."','".$data->categoria."', autor = '".$data->autor."', '".$data->descricao."')");
        if($sql){
    ...
    if($_SERVER['REQUEST_METHOD'] === 'DELETE'){
        ...
        if(isset($_GET['id'])){
            $id = $_GET['id'];
            $sql = $con->query("delete from livro where id='$id'");
            if($sql){
                exit(json_encode(array('status' => 'Sucesso')));
            }else{
                exit(json_encode(array('status' => 'Não Funcionou')));
